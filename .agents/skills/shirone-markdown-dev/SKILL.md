...
6. **manifest 登记**:语法在生产可用后才加入 `src/plugins/markdown/manifest.json`(按 `id` 排序);实现、样式、测试路径必须真实存在,`docs` 中由内容仓管理的 `src/content/posts/**` 演示文章只校验路径格式。`research/` 里的候选能力不得提前登记。插件/样式/演示/测试移动时同一提交更新 manifest。
