...
| 语法 | 写法 | Manifest ID |
|---|---|---|
| Field cards / 字段卡片 | `:::: field-group` + `::: field name` with `@type`, `@default`, and required-state metadata | `fields` |
| 提示容器 | `:::tip[标题] ... :::` 或 `> [!NOTE]`,类型:`note/info/tip/important/warning/caution/details` | `admonition` |
| 折叠面板 | `::: collapse [accordion] [expand]` 包裹无序列表,项首 `:+`/`:-` 控制开合 | `collapse-panels` |
| 选项组(Tabs) | `::: tabs[#同步id]` + `@tab 标题#值`,值相同的组跨页同步 | `option-groups` |
| 步骤流 | `:::steps{title start}` 包裹有序列表 | `steps` |
| 代码树 | `:::code-tree{title height entry icon}` 内嵌多文件代码块,或 `@[code-tree](目录)` | `code-tree` |
| 文件树 | `:::file-tree{title icon}` 嵌套列表或 ```file-tree 围栏(tree 输出) | `file-tree` |
| 马克笔高亮 | `==内容==`,变体 `==...=={.error}`(`primary/secondary/tertiary/error/tip`) | `marker` |
| 数学公式 | `$行内$` 与 `$$块级$$`(KaTeX) | `math` |
| Mermaid 图 | ` ```mermaid ` 围栏,客户端按需主题化渲染 | `mermaid` |
| 图片画廊 | `:::grid{columns="1..6" aspect="W/H" fit="cover\|contain"}` 包裹图片 | `image-grid` |
| 图片尺寸/图注 | `![说明 w-60%](src "图注")` | `image-presentation` |
| 缩写释义 | `*[SSR]: Server-Side Rendering` 定义行,悬停/聚焦/触屏出释义 | `abbreviation` |
| 内容标注 | 行内 `[+label]` 引用 + `[+label]:` 定义块,原生 Popover 展示 | `content-annotation` |
| 行内剧透 | `:spoiler[内容]` | `spoiler` |
| GitHub 卡片 | `::github{repo="owner/repo"}`,客户端按需取仓库元数据 | `github-card` |
| 文件包含 | `<!-- @include: 路径 -->`,支持 `{2-6}` 行范围与 `#region` | `include` |
| 代码块元数据 | Expressive Code:`title`、`ins={2}`、`del={3-5}`、`collapse={4-8}`、`showLineNumbers`、`frame` 等 | `expressive-code` |
...
Reference implementation and copyable example: `src/plugins/markdown/manifest.json`.
...
- `docs/markdown-extensions.md` — 处理链、样式所有权、缓存与测试契约
